...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto): Promise<UserResponseDto> {
    const existingUser = this.users.find(
      (u) => u.email.toLowerCase() === createUserDto.email.toLowerCase() && !u.archived,
    );

    if (existingUser) throw new AlreadyExistsError('User', `${createUserDto.email}`, 'email');
  ...
  async findOne(id: number): Promise<UserResponseDto> {
    ...
    if (!userEntity) throw new NotFoundError('User', String(id));
  ...
  async update(id: number, updateUserDto: UpdateUserDto): Promise<UserResponseDto> {
    ...
    if (existingUser) throw new AlreadyExistsError('User', `${updateUserDto.email}`, 'email');
